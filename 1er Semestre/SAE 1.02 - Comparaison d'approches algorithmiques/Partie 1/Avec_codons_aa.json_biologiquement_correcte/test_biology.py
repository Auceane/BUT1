...
#Question 5    
def test_codon_stop():
    assert b.codons_stop(b.load_dico_codons_aa('data/codons_aa.json'))==['UAA', 'UAG', 'UGA']
    print("Test Ok")
...
def test_codons_to_aa():
    ...
    assert b.codons_to_aa(["AGA","UAG"])==["Arginine"]
    assert b.codons_to_aa(["AGA","UAC","UAG","GAC"])==["Arginine","Tyrosine"]
    assert b.codons_to_aa(["AGA","CAG","UCG","GAC"])!=["Arginine","Serine","Serine","Aspartic acid"]
    assert b.codons_to_aa(["AGA","CAG","UCG","GAC"])==["Arginine","Glutamine","Serine","Aspartic acid"]
